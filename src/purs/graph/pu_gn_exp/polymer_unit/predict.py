...
def extract_predictions(results):
    ...
    if predictions is None:
        raise ValueError("Model did not produce global_features required for regression prediction.")
    return predictions.reshape(-1).detach().cpu().numpy()
...
if data[0].suffix == '.csv':
    ...
    source_df = dataset_predict.df.reset_index(drop=True).copy()
else:
...
with tqdm.tqdm(desc='Predict', total=len(dataloader_predict.dataset), unit='g') as bar:
    for graphs, *_ in dataloader_predict:
        ...
        results = net(graphs)
        batch_predictions = extract_predictions(results)
        ...
        if source_df is not None:
            ...
            batch_slice = source_df.iloc[i - batch_size:i].copy()
            batch_slice["Predicted_PCE"] = batch_predictions
            if "PCE_max" in batch_slice.columns:
                batch_slice["Residual"] = batch_slice["Predicted_PCE"] - batch_slice["PCE_max"]
            csv_rows.append(batch_slice)

        bar.update(graphs.num_graphs)
# endregion

if csv_rows:
    prediction_df = pd.concat(csv_rows, ignore_index=True)
    prediction_df.to_csv(options.output / "predictions.csv", index=False)
