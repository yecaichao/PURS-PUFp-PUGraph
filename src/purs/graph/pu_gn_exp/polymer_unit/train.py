...
def extract_predictions(results):
    predictions = results.global_features
    if predictions is None:
        raise ValueError("Model did not produce global_features required for regression.")
    return predictions.reshape(-1)


def resolve_artifact_dir(experiment):
    candidates = [
        experiment.get("output_dir"),
        experiment.session.checkpoint.get("folder"),
        experiment.session.log.get("folder"),
    ]
    for candidate in candidates:
        if candidate:
            ...
            return path
    fallback = Path.cwd() / "pu_gn_exp_outputs"
    fallback.mkdir(parents=True, exist_ok=True)
    return fallback


def save_regression_artifacts(output_dir, experiment, graphs_df):
    output_dir = Path(output_dir).expanduser().resolve()
    output_dir.mkdir(parents=True, exist_ok=True)

    metrics_summary = {
        "target_mse": experiment.loss_sol,
        "target_mae": experiment.target_mae,
        "target_rmse": experiment.target_rmse,
        "target_r2": experiment.target_r2,
        "num_validation_graphs": int(len(graphs_df)),
    }
    metrics_path = output_dir / "metrics_summary.json"
    metrics_path.write_text(json.dumps(metrics_summary, indent=2, ensure_ascii=False), encoding="utf-8")

    predictions_df = graphs_df.rename(
        columns={
            "Pred": "Predicted_PCE",
            "Real": "Actual_PCE",
            "Loss": "Squared_Error",
    ...
    predictions_df["Absolute_Error"] = (predictions_df["Predicted_PCE"] - predictions_df["Actual_PCE"]).abs()
...
for epoch_idx in epoch_bar:
    ...
    for graphs, targets in dataloader_train:
        ...
        targets = targets.to(experiment.session.device, dtype=torch.float32).reshape(-1)
        ...
        if experiment.session.losses.solubility > 0:
            loss_sol = criterion_mean(predictions, targets)
    ...
    for batch_idx, (graphs, targets) in enumerate(dataloader_val):
        ...
        if epoch_idx == experiment.session.epochs:
            loss_sol_by_graph = criterion(predictions, targets)
            ...
            graphs_df['Pred'].append(predictions.cpu())
...
experiment.loss_sol = float(graphs_df.Loss.mean())
experiment.target_mae = float(metrics.mean_absolute_error(graphs_df.Real, graphs_df.Pred))
experiment.target_rmse = float(np.sqrt(metrics.mean_squared_error(graphs_df.Real, graphs_df.Pred)))
experiment.target_r2 = float("nan")
if len(graphs_df) > 1:
    experiment.target_r2 = float(metrics.r2_score(graphs_df.Real, graphs_df.Pred))
artifact_dir = resolve_artifact_dir(experiment)
metrics_path, predictions_path = save_regression_artifacts(artifact_dir, experiment, graphs_df)
print('Target MSE:', experiment.loss_sol)
print('Target MAE:', experiment.target_mae)
print('Target RMSE:', experiment.target_rmse)
print('Target R2:', experiment.target_r2)
print('Metrics summary:', metrics_path.as_posix())
print('Validation predictions:', predictions_path.as_posix())
