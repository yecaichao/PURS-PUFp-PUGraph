...
def build_ring_dataframe(ring_total_list_path):
    ...
    if ring_df.empty:
        ring_df["polymer_type"] = pd.Series(dtype=str)
    else:
...
def classify_units(ring_total_list_path="ring_total_list.csv", index_data_path="index_data.csv", output_dir="."):
